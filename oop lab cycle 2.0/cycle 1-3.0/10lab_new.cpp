...
{protected:
    ...
    string name[10]={"candy","bread","rice","wheat","flour","bisciut","chips","brush","eggs","meat"};
public:
    void show_details()
    {
        for (int i = 0; i < 10; i++)
...
void list::show_list(int ct)
{
    for (int i = 0; i < ct; i++)
    {
        cout <<i+1<< ". code: " << cust_code[i]<<"Name: " <<name[i]<< ",   price: " << price_info(i)<<"   qauntity: "<<quantity[i]
